...
function Element({book, props}: any){
    return(
        <Link href={`/dashboard/${book.namespace}`}>
            <div className={`${props} hover:bg-gray-200 dark:hover:bg-gray-900 hover:z-10 hover:relative dark:text-white  transition-all ease-in-out rounded-lg p-[0.35rem] cursor-pointer`}>{book.title }</div>
        </Link>
    )
}
    ...
    return(
        <>
            ...
            {!loading && books.map((book:any, index:any) => (
                <Element props={activeChat == book.namespace ? "bg-gray-200 dark:bg-gray-900 z-10 relative" : "bg-none"} key={index} book={book}/>
            )
            )}
